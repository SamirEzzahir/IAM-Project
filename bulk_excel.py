...
def write_bulk_results(path: Path, rows: list[dict], results: list[dict]) -> None:
    ...
    headers = [
        ...
        "État", "SPL correspondant au port", "Port (Nom Usuel + NE)", "Message", "Date",
    ]
    ...
    for row, result in zip(rows, results):
        sheet.append([
            ...
            result.get("spl"), result.get("status_label"), result.get("port_spl"), result.get("msan_port"),
    ...
    widths = [12, 20, 20, 28, 10, 12, 20, 22, 24, 24, 28, 65, 25]
