...
function renderRenseigner(job) {
  ...
  $("renseignerResultsBody").innerHTML = (job.results || []).map((row) => `<tr><td>${row.excel_row}</td><td>${escapeHtml(row.input)}</td><td>${escapeHtml(row.login || "—")}</td><td>${escapeHtml(row.source || "—")}</td><td class="pco-code">${escapeHtml(row.constitution_spl || "—")}</td><td class="pco-code">${escapeHtml(row.constitution_pco || "—")}</td><td>${escapeHtml(row.constitution_brin || "—")}</td><td class="pco-code">${escapeHtml(row.msan_port || "—")}</td><td><span class="row-status ${statusClass(row.status)}">${escapeHtml(row.status_label)}</span></td><td>${row.duration_seconds == null ? "—" : `${Number(row.duration_seconds).toFixed(1)} s`}</td><td class="message-cell">${escapeHtml(row.message || "—")}</td></tr>`).join("") || '<tr><td colspan="11" class="empty"><strong>Aucune collecte lancée</strong></td></tr>';
